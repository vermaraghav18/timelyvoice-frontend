...
export default function TopV2({ section }) {
  ...
  const bottomCardsRaw =
    Array.isArray(items.belowGrid) && items.belowGrid.length
      ? items.belowGrid
      : Array.isArray(items.sideStack)
      ? items.sideStack
      : [];

  // ✅ render exactly 4 (what you asked for)
  const bottomCards = useMemo(() => bottomCardsRaw.slice(0, 4), [bottomCardsRaw]);
      ...
      {hero ? (
        <div className="topv2-hero">
          ...
          <div className="topv2-hero-text">
            {heroTime ? (
              <div className="topv2-updated">{`UPDATED ${heroTime.toUpperCase()}`}</div>
            ) : null}
